...
namespace NzbDrone.Common.Test.DiskTests
{
    ...
    public class DirectoryLookupServiceFixture : TestBase<FileSystemLookupService>
    {
        ...
        public void should_not_contain_recycling_bin_for_root_of_drive()
        {
            ...
            Subject.LookupContents(root, false, false).Directories.Select(d => d.Name).Should().NotContain(Path.Combine(root, RECYCLING_BIN));
        }
        ...
        public void should_not_contain_system_volume_information()
        {
            ...
            Subject.LookupContents(root, false, false).Directories.Select(d => d.Name).Should().NotContain(Path.Combine(root, SYSTEM_VOLUME_INFORMATION));
        }
